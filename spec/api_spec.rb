...
describe RubyBox::API, '#list' do
  it "returns list of items in the root folder if no arguments given" do
    ruby_box = RubyBox::API.new(ACCOUNT['api_key'], ACCOUNT['auth_token'])
  ...
  it "returns appropriately if the path does not exist" do
    ruby_box = RubyBox::API.new(ACCOUNT['api_key'], ACCOUNT['auth_token'])
    expect { ruby_box.list(207492335) }.to raise_error(RubyBox::ObjectNotFound)
  end
end

describe RubyBox::API, '#list_by_path' do
...
describe RubyBox::API, '#put_file' do
...
describe RubyBox::API, '#put_file_in_folder_path' do
